...
```{r}
p_value_right_subset1 <- permutation_pvalue_right(ptest_stats_subset1, observed_value_subset1)
p_value_left_subset1 <- permutation_pvalue_left(ptest_stats_subset1, observed_value_subset1)
cat(p_value_right_subset1, p_value_left_subset1)

p_value_right_subset2 <- permutation_pvalue_right(ptest_stats_subset2, observed_value_subset2)
p_value_left_subset2 <- permutation_pvalue_left(ptest_stats_subset2, observed_value_subset2)
cat(p_value_right_subset2, p_value_left_subset2)
```
On peut d'abord supposer que, Dans le cas du test droite, la probabilité de la valeur de permutation est moins que la valeur observé, et que, dans le cas du test gauche, la probablitié de la valeur de permutation est plus que la valeur observé.  

Et, On peut savoir les deux résultats sur deux tests. D'abord, dans le subset 1 et 2, les p-valeurs gauches sont inférieures aux p-valeurs droites. (pour le subset 1, 0.779878 >  0.2703270 / pour le subset 2, 0.9405941>0.1272127). 
dans le cas des p_values droites, le subset 2 est supérieur au subset 1 (0.9405941 > 0.779878). Et, on peut supposer que la valeur observée du subset 2 ne peut bien pas observer dans le test de permutation.
...
C'est ici où on epeut dire qu'il est significant car la p-value est 5.974e-05.  
...
for (size in sample_size) {
  for (mean in diff_in_mean) {
    for (sd in s_d) {
      N_SAMPLES <- 9999
      n_successes <- 0
      for (i in N_SAMPLES){
        sample_1 <- rnorm(size, 3, sd)
        sample_2 <- rnorm(size, 3+mean, sd)
        df <- tibble::data_frame(value=c(sample_1, sample_2), group=c(rep("A", size), rep("B",size)))
