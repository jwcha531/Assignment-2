...
sample_sizes <- c(5,50)
means <- c(1,2)
stardard_deviations <- c(2,6)

alpha <- 0.01
power_df <- NULL
for (size in sample_sizes){
  for (mean in means){
    for (deviation in stardard_deviations){
      n_successes <- 0
      for(i in 1:1000){
          a <- data.frame("group"= "A", "value" = rnorm(size, mean, deviation))
          b <- data.frame("group"= "B", "value" = rnorm(size, mean, deviation))
...
ggplot2::ggplot(power_df, 
                ...
                ggplot2::labs(title="Size Plot", x="Size", y="Power") + 
                ggplot2::geom_count(colour="yellow") +
                ggplot2::theme_dark()

ggplot2::ggplot(power_df, 
                ggplot2::aes(x=mean, y=power)) +
                ggplot2::labs(title="Mean Plot", x="Mean", y="Power") + 
                ggplot2::geom_count(colour="pink")+
                ggplot2::theme_dark()

ggplot2::ggplot(power_df, 
                ggplot2::aes(x=deviation, y=power)) +
                ggplot2::labs(title="Ecart typet", x="Ecart type", y="Power") + 
